...
import { defaultWeatherProps, WeatherContext } from './WeatherContext'

// Holds a list of weather bubbles,
// takes zip codes, and creates
// new bubbles
function WeatherPane() {

  
  const [weatherData,setWeatherData] = useState(defaultWeatherProps)


  let newBubble :JSX.Element = ( <WeatherBubbleWithProps {...defaultWeatherProps} key='WeatherBubble-0'/> )
  
  // List of weather bubbles displayed on the pane
  ...
  async function getWeatherData(zipCode : string) {
    ...
    const geoRes = await fetch('http://api.openweathermap.org/geo/1.0/zip?zip=' + zip + '&appid=' + import.meta.env.VITE_WEATHER_API_KEY)
      ...
      if(bubbles[0] === newBubble ){
          
        console.log( 'replace starter' )
        // unmountComponentAtNode( bubbles[0] )         
        setBubbles( [ < WeatherBubbleWithProps {...weatherData} /> ] )
        console.log(bubbles[0])

      }else {
        setBubbles( [...bubbles, <WeatherBubbleWithProps {...weatherData} key={`WeatherBubble-{bubbles.length}`} /> ] )
      }
  }
  ...
  return (
    ...
    <div className='weather-panel'>
      <input type="text" 
              ...
              {bubbles.map( (bubble) => {
                return bubble == newBubble ? '' : bubble
              })}
